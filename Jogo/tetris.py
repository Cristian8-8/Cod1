...
class Jogo:
    def __init__(self,tam_cel): 
        ...
        self.largura = self.janela.get_width()
        self.altura = self.janela.get_height()
        ...
        self.inimigos = []
        self.vida_torre= 100
        self.torre_pos = (self.largura//2 - 20,30)
        self.linhas_inimigas = 0
        self.altura_td=120
        self.altura_tabuleiro= self.altura - self.altura_td
        self.timer_noite=0
        self.intervalo_spawn= 180
        self.inimigos_por_noite=5
        self.inimigos_restantes=0
    ...
    def passar_turno(self):
        ...
        if self.turnos_fase >= 10:
            self.turnos_fase = 0
            self.dia = not self.dia

        if not self.dia:  # começou a noite
            self.timer_noite = 0
            self.inimigos_restantes = self.inimigos_por_noite
        else:  # começou o dia
            self.fim_noite()
    def adicionar_garbage(self):
        buraco=random.randint(0,9)
        linha=[]
        for x in range(10):
            if x == buraco:
                linha.append('')
        ...
        if self.posicao_forma[1]>0:
            self.posicao_forma[1] -= 1
    def remover_garbage_superior(self):
        for y in range(20):
            if 'X' in self.mapa[y]:
                del self.mapa[y]
    ...
    def atualizar_noite(self):
        if not self.dia and not self.exibi_restart and self.inimigos_restantes > 0:
            self.timer_noite += 1

            if self.timer_noite >= self.intervalo_spawn:
                self.spawn_inimigo()
    ...
    def fim_noite(self):
        if self.inimigos:
            dano=len(self.inimigos)*10
            self.vida_torre -= dano
            self.inimigos.clear()
        if self.vida_torre <= 0:
    ...
    def forma_aleatoria(self):
        return random.choice(list(self.formas.keys()))
    ...
    def desenhar_torre(self):
        x,y=self.torre_pos
        pygame.draw.rect(self.janela,(120,120,255),(x,y,40,60))
    ...
    def desenhar_area_td(self):
        pygame.draw.rect(self.janela,(20, 20, 20),(0, 0, self.largura, self.altura_td))
        pygame.draw.line(self.janela,self.branco,(0, self.altura_td),(self.largura, self.altura_td),2)
    ...
    def recb_cor(self, cod_cor): # essa função recebe um codigo de cor e retorna uma cor
        if cod_cor == 'R':        # se o codigo da cor for 'R' retorna a cor 'roxa'
            ...
            return self.verde     
        elif cod_cor == 'B' or cod_cor == 'X':      # se o codigo da cor for 'B' retorna a cor 'Cinza'. Cinza = B pois ja existe um 'C'
            return self.cinza
        else:
    ...
    def adicionar_pontos(self,linhas):# adiciona pontos conforme as linhas completadas
        ...
        self.tempo = 0 # zera o tempo do jogo
